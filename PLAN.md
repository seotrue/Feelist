...
### 5단계: Mock 데이터 & React Query 훅 (mock 기반)
> 백엔드 없이 UI를 먼저 완성하는 전략 — React Query를 mock 데이터로 연결한 뒤 나중에 실제 API로만 교체

- [ ] `src/mocks/analysis.ts`: MoodAnalysis 가짜 데이터 (다양한 무드 시나리오)
- [ ] `src/mocks/tracks.ts`: SpotifyTrack[] 가짜 트랙 리스트 (앨범아트 포함)
- [ ] `src/hooks/useAnalyze.ts`: 분석 mutation 훅 (mock 데이터 반환)
- [ ] `src/hooks/usePlaylist.ts`: 플레이리스트 생성 mutation 훅 (mock 데이터 반환)
- [ ] TanStack Query Provider 설정

### 6단계: 프론트엔드 UI (React Query + mock 데이터 기반)
...
- [ ] `page.tsx`: mock 데이터로 메인 페이지 조합

### 7단계: 비즈니스 컴포넌트 스토리 추가
- [ ] MoodInput, TrackItem, PlaylistCard 스토리 작성
- [ ] mock 데이터 활용한 인터랙션 시나리오 문서화
...
### 9단계: mock → 실제 API 교체
- [ ] `useAnalyze.ts`: mock 반환 → `/api/analyze` 호출로 교체
- [ ] `usePlaylist.ts`: mock 반환 → `/api/playlist` 호출로 교체
- [ ] Zustand 인증 상태 연동
